...
function SearchInput(props: { onClickHandler: (inputValue: string) => any }) {
  ...
  const EnterPressHandler = (e: React.KeyboardEvent) =>{
    if(e.key==='Enter' && inputRef.current) onClickHandler(inputRef.current.value)
  }
  return (
    <InputGroup size="md">
      <Input pr="4.5rem" type="text" placeholder="Customer Name" ref={inputRef} onKeyPress={EnterPressHandler}/>
